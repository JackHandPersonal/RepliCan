...
$src = "C:\Dev\Games\RepliCan\RawArt\Props"
...
for ($s = 0; $s -lt $sheets; $s++) {
  ...
  $g.Clear($back)
  $g.DrawString(("CyberCity cafeteria props  -  sheet {0} of {1}" -f ($s + 1), $sheets), $font, $dim, 10, 8)
  ...
  foreach ($f in $slice) {
    ...
    $name = $f.BaseName -replace '^SM_Prop_', ''
  ...
  $out = Join-Path $dst ("PropSheet_{0}.png" -f ($s + 1))
