...
$src = "C:\Dev\Games\RepliCan\RawArt\Grips"
...
for ($s = 0; $s -lt $sheets; $s++) {
  ...
  $g.Clear($back)
  $g.DrawString(("Weapons held, third person then first  -  sheet {0} of {1}" -f ($s + 1), $sheets), $font, $dim, 10, 8)
  ...
  foreach ($f in $slice) {
    ...
    $name = $f.BaseName -replace '__third$', ' 3p' -replace '__first$', ' 1p'
  ...
  $out = Join-Path $dst ("GripSheet_{0}.png" -f ($s + 1))
